...
def extend_class(cls: Type[ExchangeCalendar], day_of_week_expiry: Optional[int] = None,
    ...
    def special_closes(self) -> List[Tuple[datetime.time, Union[HolidayCalendar, int]]]:
        return [(t, rules if type(rules) == int else HolidayCalendar(rules=rules)) for t, rules in self._adjusted_properties.special_closes]
    ...
    def special_opens(self) -> List[Tuple[datetime.time, Union[HolidayCalendar, int]]]:
        return [(t, rules if type(rules) == int else HolidayCalendar(rules=rules)) for t, rules in self._adjusted_properties.special_opens]
